...
async def update_quiz(quiz_id: str, title: str, user_id: str, supabase: Client):
    ...
    try:
        ...
        if not response.data:
            ...
            raise HTTPException(status_code=404, detail="Quiz not found or you don't have permission to update it.")



        # Update

        update_res = supabase.table("quizzes").update({"title": title}).eq("id", quiz_id).execute()

        

        if not update_res.data:

             raise Exception("Failed to update quiz record")

             

    except Exception as e:

        logger.error(f"Error updating quiz {quiz_id} for user {user_id}: {e}")
